...
url = f'{base}/wiki/Final_Fantasy_XII_accessories'
...
async def get_accessories():
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as response:
            if response.status != OK:
                raise RuntimeError("Could not get accessories.")
    ...
    for table in tables:
        results.extend(_read_accessories_table(table))

    return results


def _read_accessories_table(table: Tag):
...
async def main():
    accessories = await get_accessories()
